...
/-!
Smoke test for `CertChecker.checkCertificateJson`, run against
`certificate_builder.build_certificate` output via `#eval`: this runs the
program against files on disk, not a compile-time proof about literals.
...
decidability instance is exponential (it enumerates every walk, with no
visited-set pruning; see `ForestDecide.lean`'s docstring). That instance has
been replaced with an incremental union-find over `Finset V` partitions
(`mergeStep`/`buildComponents`), reused as a components cache across a
piece's whole maximality check. `nested` now completes and reports
`ACCEPTED`, verified directly with `lake exe verify_cert
../cpp/examples/nested.certificate.json` in roughly 2.5 seconds, compiled.
Lean's interpreter (what `#eval` runs under, as opposed to `verify_cert`'s
compiled code) still carries constant-factor overhead per primitive
operation, but `nested`'s `#eval` now finishes in about 8 seconds rather
than the 3+ minutes it used to take under the old exponential instance:
comfortably within a smoke-test budget, so it's re-enabled below alongside
`house`/`branch_test`.
-/
