...
This project's one deliberately accepted soundness gap. Checking that a
density `ρ` is admissible (every spanning tree has `ρ`-weight `≥ 1`)
reduces to checking that the *minimum*-weight spanning tree's weight is
`≥ 1`: if the minimum clears the bar, every tree does, since none can
weigh less than the true minimum. Finding that minimum is exactly what
Kruskal's algorithm computes, but this implementation's output is not
proven to be a minimum spanning tree; the checker only runs it and trusts
the result. That is the whole content of the gap; nothing here tries to
paper over it.

Because this computation's correctness is out of scope for what Lean
checks (by design, a planned follow-up, not an oversight), there is no
proof obligation riding on this file at all: `find`'s termination isn't
...
neither is `run` producing a minimum spanning tree. This is the same class
...
compression: simplicity over speed, since this whole computation already
sits outside what Lean verifies (see the file docstring), so a faster
version would buy nothing but speed, and Kruskal's comparison-sort step
already dominates asymptotically. `partial`: proving termination needs an
invariant (e.g. `parent`'s root-chain length strictly decreasing) that
this file has no other use for, and is moot anyway given the whole
computation is untrusted. -/
...
docstring): its result is trusted to be a minimum spanning tree of a
connected graph, not proven to be one. -/
